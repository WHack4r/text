...
export default {
  ...
  methods: {
    ...
    removeCategoryName() {
      ...
      this.getData();
      // 地址栏也需要修改：进行路由跳转
      // 判断是否还有params参数(本意是删除query，不应该删除params参数，路由跳转的时候应该带过去)
  ...
  // 数据监听：监听实例身上的属性的属性值的变化
  watch: {
    // 监听路由的信息是否发生变化，如果发生变化，再次发起请求
    $route(newVaue, oldValue) {
      // 再次发请求之前整理带给服务器参数
      Object.assign(this.searchParams, this.$route.query, this.$route.params);
      // 再次发起ajax请求
      this.getData();
